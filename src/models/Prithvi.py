...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_pred = nn.Linear(
            decoder_embed_dim, tubelet_size * patch_size[-1] * patch_size[1] * in_chans, bias=True
        )  # decoder to patch
        # --------------------------------------------------------------------------
    ...
    def unpatchify(self, x):
        ...
        """
        p = self.patch_embed.patch_size[1]
        num_p = self.patch_embed.img_size[1] // p
        tub = self.patch_embed.tubelet_size
        imgs = rearrange(
            x,
    ...
    def forward(self, x):
        ...
        # add positional embedding directly without mask handling
        x = x + self.decoder_pos_embed
