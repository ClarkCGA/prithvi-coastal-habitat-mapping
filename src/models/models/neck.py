...
class FPN1_TransposedConv(nn.Module):
    def __init__(self,in_channel,out_channel, patch_size):
        super(FPN1_TransposedConv, self).__init__()
...
class FPN1_Bilinear(nn.Module):
    def __init__(self, in_channel, out_channel, patch_size):
        super(FPN1_Bilinear, self).__init__()
...
class FPN1_PixelShuffle(nn.Module):
    def __init__(self, in_channel, out_channel, patch_size):
        super(FPN1_PixelShuffle, self).__init__()
...
class Neck(nn.Module):
    def __init__(self,embed_size, patch_size, fpn1_type="transposed"):
        super(Neck, self).__init__()
        
        self.embed_dim=embed_size
        if fpn1_type == "transposed":
            self.fpn1 = FPN1_TransposedConv(self.embed_dim, self.embed_dim, patch_size)
        elif fpn1_type == "bilinear":
            self.fpn1 = FPN1_Bilinear(self.embed_dim, self.embed_dim, patch_size)
        elif fpn1_type == "pixelshuffle":
            self.fpn1 = FPN1_PixelShuffle(self.embed_dim, self.embed_dim, patch_size)
        else:
            raise ValueError(f"Unknown fpn1_type '{fpn1_type}'. Choose from 'transposed', 'bilinear', 'pixelshuffle'.")
